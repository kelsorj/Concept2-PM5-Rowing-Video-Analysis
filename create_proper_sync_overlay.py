#!/usr/bin/env python3
"""
Create properly synchronized overlay video using raw data
"""
...
def create_proper_sync_overlay(video_path, raw_csv_path, frames_csv_path, output_dir):
    """Create overlay video with proper timestamp synchronization"""
    print("🎬 Creating Properly Synchronized Overlay Video")
    print("=" * 60)
    ...
    # Calculate time offset between frames and force data
    first_frame_dt = frame_timestamps[0]['timestamp_dt']
    first_force_dt = force_data[0]['timestamp_dt']
    time_offset = (first_force_dt - first_frame_dt).total_seconds()
    
    print(f"📊 Time offset between frames and force data: {time_offset:.3f}s")
    ...
    output_path = os.path.join(output_dir, f"rowing_proper_sync_overlay_{timestamp}.mp4")
    
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_path, fourcc, fps, (width, height))
    
    print(f"📊 Processing frames with proper synchronization...")
    ...
    while cap.isOpened():
        ...
        if frame_count <= len(frame_timestamps):
            ...
            # Find closest force data using elapsed time + offset
            closest_force = None
            min_time_diff = float('inf')
            
            for force_entry in force_data:
                # Use elapsed time from force data for comparison
                force_elapsed = force_entry['elapsed_s']
                time_diff = abs(elapsed_s - force_elapsed)
            ...
            # Track sync errors
            if min_time_diff > 0.5:  # More than 500ms difference
...
def main():
    parser = argparse.ArgumentParser(description="Create properly synchronized overlay video")
    ...
    parser.add_argument("--output-dir", default="proper_sync_analysis", help="Output directory")
    ...
    create_proper_sync_overlay(args.video, args.raw_csv, args.frames_csv, args.output_dir)
