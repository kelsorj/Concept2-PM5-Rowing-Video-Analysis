#!/usr/bin/env python3
"""
Correlate Force Data with Body Pose Angles using Elapsed Time
Matches force curves with body angles based on elapsed time from start
"""
...
def find_closest_pose_by_elapsed_time(force_elapsed_s, pose_data, time_window=1.0):
    """Find the closest pose frame by elapsed time"""
    closest_frame = None
    min_time_diff = float('inf')
    
    for frame in pose_data:
        pose_elapsed_s = frame.get('timestamp', 0)  # This is elapsed time in seconds
        
        time_diff = abs(force_elapsed_s - pose_elapsed_s)
...
def analyze_force_pose_correlation(force_data, pose_data):
    """Analyze correlation between force curves and body angles using elapsed time"""
    print("\n🔍 Analyzing Force-Pose Correlations by Elapsed Time...")
    
    correlations = []
    
    for i, force_entry in enumerate(force_data):
        print(f"   Processing force curve {i+1}/{len(force_data)} (t={force_entry['elapsed_s']:.1f}s)")
        
        # Find closest pose frame by elapsed time
        closest_pose, time_diff = find_closest_pose_by_elapsed_time(
            force_entry['elapsed_s'], pose_data
        )
        ...
        pose_stats = {
            ...
            'pose_elapsed_s': closest_pose.get('timestamp', 0)
        }
        
        # Combine data
        correlation_entry = {
            'force_elapsed_s': force_entry['elapsed_s'],
            'time_diff': time_diff,
...
def create_correlation_plots(correlations):
    ...
    fig.suptitle('Force vs Body Angles Correlation (by Elapsed Time)', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('correlation_plots/force_vs_angles_elapsed.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 2. Force Curves with Body Angles Overlay
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Force Curves with Body Angles (by Elapsed Time)', fontsize=16)
    ...
    for i, idx in enumerate(sample_indices):
        if idx < len(correlations):
            ...
            if force_curve:
                ...
                axes[row, col].set_title(f'Drive {idx+1} - t={corr["force_elapsed_s"]:.1f}s, Peak: {corr["peak_force"]:.1f}')
                axes[row, col].grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('correlation_plots/force_curves_with_angles_elapsed.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 3. Time Series of Force and Angles
    fig, axes = plt.subplots(3, 1, figsize=(16, 12))
    fig.suptitle('Force and Body Angles Over Elapsed Time', fontsize=16)
    
    # Plot peak force over time
    axes[0].plot(df['force_elapsed_s'], df['peak_force'], 'b-', linewidth=2, label='Peak Force')
    axes[0].set_ylabel('Peak Force')
    axes[0].set_title('Peak Force Over Elapsed Time')
    ...
    if 'left_arm_angle' in df.columns and 'right_arm_angle' in df.columns:
        ...
        if len(valid_arm_data) > 0:
            axes[1].plot(valid_arm_data['force_elapsed_s'], valid_arm_data['left_arm_angle'], 
                        'g-', linewidth=2, label='Left Arm')
            axes[1].plot(valid_arm_data['force_elapsed_s'], valid_arm_data['right_arm_angle'], 
                        'r-', linewidth=2, label='Right Arm')
            axes[1].set_ylabel('Arm Angle (degrees)')
            axes[1].set_title('Arm Angles Over Elapsed Time')
    ...
    if 'left_leg_angle' in df.columns and 'right_leg_angle' in df.columns:
        ...
        if len(valid_leg_data) > 0:
            axes[2].plot(valid_leg_data['force_elapsed_s'], valid_leg_data['left_leg_angle'], 
                        'g-', linewidth=2, label='Left Leg')
            axes[2].plot(valid_leg_data['force_elapsed_s'], valid_leg_data['right_leg_angle'], 
                        'r-', linewidth=2, label='Right Leg')
            axes[2].set_xlabel('Elapsed Time (seconds)')
            axes[2].set_ylabel('Leg Angle (degrees)')
            axes[2].set_title('Leg Angles Over Elapsed Time')
            axes[2].grid(True, alpha=0.3)
            axes[2].legend()
    
    plt.tight_layout()
    plt.savefig('correlation_plots/force_angles_timeseries_elapsed.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def save_correlation_data(correlations):
    ...
    output_file = 'force_pose_correlations_elapsed.csv'
...
def main():
    """Main correlation analysis using elapsed time"""
    print("🚣‍♂️ Force-Pose Correlation Analysis (Elapsed Time)")
    print("=" * 60)
    ...
    # Show time ranges
    if force_data:
        force_times = [f['elapsed_s'] for f in force_data]
        print(f"   Force data time range: {min(force_times):.1f}s - {max(force_times):.1f}s")
    
    if pose_data:
        pose_times = [p.get('timestamp', 0) for p in pose_data]
        print(f"   Pose data time range: {min(pose_times):.1f}s - {max(pose_times):.1f}s")
