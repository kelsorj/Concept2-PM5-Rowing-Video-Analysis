#!/usr/bin/env python3
"""
Create Complete Overlay Video
Combines proper force animation with body angle overlays
"""
...
def create_complete_overlay_video(video_path, raw_csv_path, pose_json_path, output_dir="complete_overlay"):
    """Create complete overlay video with proper force animation and body angles"""
    ...
    # Find when rowing actually starts
    first_stroke_start = combined_strokes[0]['start_timestamp_dt']
    time_to_first_stroke = (first_stroke_start - video_start_dt).total_seconds()
    print(f"⏰ Rowing starts at: {first_stroke_start.strftime('%H:%M:%S.%f')[:-3]} ({time_to_first_stroke:.1f}s into video)")
    
    # Create output video
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = os.path.join(output_dir, f"complete_overlay_{timestamp}.mp4")
    
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_path, fourcc, fps, (width, height))
    
    print(f"📊 Processing frames with complete overlays...")
    
    frame_count = 0
    overlays_added = 0
    force_overlays = 0
    rowing_started = False
    ...
    while cap.isOpened():
        ...
        # Check if we've reached the rowing start time
        if current_frame_abs_time >= first_stroke_start:
            rowing_started = True
        ...
        # Only show force curves when rowing has started
        if rowing_started:
        ...
        # Add frame info with rowing status
        ...
        if rowing_started:
            ...
            time_until_rowing = (first_stroke_start - current_frame_abs_time).total_seconds()
            info_text += f" | Rowing starts in {time_until_rowing:.1f}s"
    ...
    print(f"\n🎉 Complete overlay video created!")
    ...
    print(f"   ⏰ Rowing started at frame: {int(time_to_first_stroke * fps)}")

def main():
    parser = argparse.ArgumentParser(description="Create complete overlay video with proper force animation and body angles")
    ...
    parser.add_argument("--output-dir", default="complete_overlay", help="Output directory")
    
    args = parser.parse_args()
    
    print("🎬 Creating Complete Overlay Video with Proper Force Animation and Body Angles")
    print("=" * 80)
    
    create_complete_overlay_video(args.video, args.raw_csv, args.pose_json, args.output_dir)
