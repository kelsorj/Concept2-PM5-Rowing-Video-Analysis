...
class ComprehensiveStrokeAnalysis:
    ...
    def load_stroke_data_for_3d(self, analysis_dir):
        """Load pose data and combined strokes for 3D visualization"""
        print(f"📊 Loading data for 3D visualization from: {analysis_dir}")
    ...
    def create_3d_visualizations(self, analysis_dir):
        """Create all 3D visualizations: individual animations, combined analysis, and interactive dashboard"""
        print("🎬 Creating 3D Visualizations")
        print("=" * 50)
        
        data = self.load_stroke_data_for_3d(analysis_dir)
        if not data:
            return
        ...
        # 1. Create individual stroke animations
        print("📹 Creating individual stroke animations...")
        ...
        for stroke_num in range(1, num_strokes + 1):
            ...
            result = self.create_3d_stroke_animation(analysis_dir, stroke_num, output_path)
            if result:
                generated_files.append(result)
        
        # 2. Create combined analysis
        print("📊 Creating combined stroke analysis...")
        combined_path = self.create_combined_stroke_analysis(analysis_dir)
        if combined_path:
            generated_files.append(combined_path)
        
        # 3. Create interactive dashboard
        print("🎛️ Creating interactive axis dashboard...")
        dashboard_path = self.create_interactive_axis_dashboard(analysis_dir)
        if dashboard_path:
            generated_files.append(dashboard_path)
        
        print(f"\n🎉 Generated {len(generated_files)} 3D visualizations!")
        print(f"📁 Output directory: {output_dir}")
        print(f"\n💡 Open any HTML file in your browser to view the interactive 3D visualizations")
        
        return generated_files
    
    def create_3d_stroke_animation(self, analysis_dir, stroke_number, output_path):
    ...
    def create_combined_stroke_analysis(self, analysis_dir):
    ...
    def create_interactive_axis_dashboard(self, analysis_dir):
        """Create an interactive dashboard where users can choose which angle goes on which axis"""
        print("🎛️ Creating interactive axis dashboard...")
        
        # For now, create a simple placeholder that redirects to the standalone tool
        output_dir = os.path.join(analysis_dir, "3d_animations")
        os.makedirs(output_dir, exist_ok=True)
        
        # Create a simple HTML file that explains how to use the interactive dashboard
        html_content = """
        <!DOCTYPE html>
        <html>
        <head>
            <title>Interactive Dashboard - Use Standalone Tool</title>
            <style>
                body { font-family: Arial, sans-serif; margin: 40px; }
                .info { background-color: #e8f4f8; padding: 20px; border-radius: 10px; border-left: 4px solid #2196F3; }
                .command { background-color: #f0f0f0; padding: 10px; border-radius: 5px; font-family: monospace; }
            </style>
        </head>
        <body>
            <h1>Interactive Axis Dashboard</h1>
            <div class="info">
                <h3>To create the interactive dashboard with axis selection:</h3>
                <p>Run the standalone 3D animation tool with the interactive flag:</p>
                <div class="command">python create_3d_stroke_animation.py analysis_py3rowcap_20251004_123520 --interactive</div>
                <p>This will create an interactive dashboard where you can select which angle measurements to display on each axis.</p>
            </div>
        </body>
        </html>
        """
        
        output_path = os.path.join(output_dir, "interactive_dashboard_info.html")
        with open(output_path, 'w') as f:
            f.write(html_content)
        
        print(f"   ✅ Created info page: {os.path.basename(output_path)}")
        return output_path
...
def main():
    ...
    generator.create_3d_visualizations(args.analysis_dir)
