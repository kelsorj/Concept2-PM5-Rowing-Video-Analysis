#!/usr/bin/env python3
"""
Create Synchronized Overlay Video with Corrected Force Data
Combines the corrected force data processing with video overlay and kinematics analysis
"""
...
def create_force_curve_plot(force_curve, power, spm, current_idx=None, stroke_num=None):
    """Create a matplotlib plot of the force curve with an animated dot"""
    if not force_curve:
        return None
...
def find_closest_stroke_for_time(target_time, combined_strokes):
    """Find the closest stroke for a given timestamp"""
    ...
    return closest_stroke, closest_idx

def create_synchronized_overlay_video(video_path, raw_csv_path, pose_json_path, output_dir="synchronized_overlay_analysis"):
    """Create synchronized overlay video with corrected force data and pose analysis"""
    ...
    output_path = os.path.join(output_dir, f"synchronized_overlay_{timestamp}.mp4")
    
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_path, fourcc, fps, (width, height))
    
    print(f"📊 Processing frames with synchronized overlays...")
    ...
    while cap.isOpened():
        ...
        closest_stroke, force_idx = find_closest_stroke_for_time(current_frame_abs_time, combined_strokes)
        ...
        if closest_stroke and closest_stroke['combined_forceplot']:
            ...
            force_plot = create_force_curve_plot(
                ...
                force_idx,
                stroke_num=combined_strokes.index(closest_stroke) + 1
            )
    ...
    print(f"\n🎉 Synchronized overlay video complete!")
...
def main():
    parser = argparse.ArgumentParser(description="Create synchronized overlay video with corrected force data")
    ...
    parser.add_argument("--output-dir", default="synchronized_overlay_analysis", help="Output directory")
    
    args = parser.parse_args()
    
    print("🎬 Creating Synchronized Overlay Video with Corrected Force Data")
    print("=" * 70)
    
    create_synchronized_overlay_video(args.video, args.raw_csv, args.pose_json, args.output_dir)
